...
- Фикстуры из `tests/conftest.py`: `client`, `conn`, `freeze_time`, автоочистка таблиц после каждого теста. Любой тест, где важна дата (неделя, дедлайн, recency), замораживает время через `freeze_time`, а не подбирает «сегодня». Данные создаются через `tests/factories.py` (raw SQL), не через ручки, если только сама ручка не тестируется.
...
`mypy` проверяет `app`, `tests` и `scripts` одинаково: тесты и фикстуры аннотированы полностью, `Any` допустим только в обёртках над драйвером (`tuple[Any, ...]` из курсора).
