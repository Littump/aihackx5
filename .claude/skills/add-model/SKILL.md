...
4. **Модель строки.** В `app/features/<feature>/models.py` — `class XRow(AppModel)` (база из `app/core/models.py`) с полями один-в-один с колонками и теми же именами. JSONB — вложенная модель, не `dict`. `Decimal` для `NUMERIC`, `datetime` для `TIMESTAMPTZ`, `Literal[...]` для колонок с `CHECK IN`.
