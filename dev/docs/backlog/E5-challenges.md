...
**Найдено при FE-D04 (перерисовка фронта, 2026-09-04):** текущая заглушка `domovoy_copy.py` (из BE-011) подставляет английские ключи как есть — `title=f"Больше {category}"` даёт «Больше bakery», `insight` даёт «За month ты сэкономил…». Категории и период нужно локализовать на русский (маппинг категорий уже есть на фронте в `dev/frontend/src/features/home/categoryLabels.ts` — можно ориентироваться на те же названия для консистентности между PM view и продуктовыми экранами). Актуально и после LLM-планировщика (E14-ml-planner) — fallback-путь никуда не денется.
