...
**Найдено при FE-D06 (перерисовка фронта, 2026-09-04):** `GET /users/{id}/league` отдаёт `promotion_cutoff`/`demotion_cutoff` как чистые числа ранга (`scoring.promotion_cutoff/demotion_cutoff`), без учёта того, что в дивизионе 1 понижаться некуда, а в дивизионе 5 — повышаться (это уже учтено в `scoring.zone_for_rank` для `my_zone`, но не для отображения границ зон в списке участников). Фронт (`features/league/components/LeaderboardList.tsx`) поэтому не может сам решить, показывать ли разделитель «Выше — повышение» в дивизионе 5 или «Ниже — понижение» в дивизионе 1 — константа `LEAGUE_DIVISIONS_COUNT` живёт только в `game_rules.py` и дублировать её на фронте нельзя. Нужно расширить контракт (`LeagueResponse`) полями вроде `can_promote`/`can_demote` (bool), считающимися так же, как в `scoring.zone_for_rank`, чтобы фронт мог скрыть неактуальный разделитель на границах дивизионной лестницы.
