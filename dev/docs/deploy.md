...
Миграции идут только вперёд, отката схемы нет. `deploy/rollback.sh <sha>` откатывает только код: выставляет `TAG=<sha>`, `pull` и `up -d` для `postgres`, `backend`, `frontend` — `migrate` не запускается.

Откат кода допустим только если миграция, применённая после отменяемого релиза, обратно совместима со старым кодом — иначе старый backend не сможет работать с новой схемой. Если несовместима, откатывать нельзя, нужен fix-forward.
