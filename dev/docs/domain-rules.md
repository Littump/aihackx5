...
Объяснимость цифры (для UI): `receipts_count` — сколько counted-чеков попало в период; у каждой категории `items_count` — сколько позиций дали ненулевой вклад, и `top_products` — до `SAVINGS_TOP_PRODUCTS_LIMIT = 3` названий товаров с наибольшим вкладом (одноимённые позиции суммируются, порядок при равенстве — по алфавиту).
...
### 6.1 Маржинальность категорий (ML-планировщик)

Помимо базовой `CONTRIBUTION_MARGIN = 0.15`, ML-планировщик знает про-категорийную маржу `CATEGORY_CONTRIBUTION_MARGIN` (assumptions симуляции, `game_rules.py`), чтобы «зарабатывать больше» с покупателя. Категория считается высокомаржинальной, если её маржа `>= HIGH_MARGIN_MIN_MARGIN = 0.28`.
...
| snacks | 0.30 |
| drinks | 0.28 |
| alcohol | 0.25 (не участвует в челленджах) |
| fruits_veg | 0.20 |
| other | 0.18 |
| bakery | 0.16 |
| dairy | 0.14 |
...
Правило планировщика (A/B, `ML_HIGH_MARGIN_MANDATE_ENABLED`): когда включено, план обязан содержать хотя бы один челлендж по высокомаржинальной категории (обычно side-квест поверх hero); при выключенном флаге требование снимается.
...
| Названия позиций | из каталога `receipts/catalog.py`, без повторов внутри категории | — |
| Цены в черновике | округлены до целых рублей | — |
...
## 17. Баланс баллов и история начислений

`GET /users/{id}/rewards` отвечает на «сколько у меня баллов и за что их дали». `points_balance` дублируется в `GET /home`, чтобы баланс был виден на первом экране.

```
points_balance = Σ reward_ledger.points_delta                       # награды за цели и рефералов
               + Σ (points_earned − points_spent) по counted-чекам  # баллы кассы X5, если прислала
```

Возвращённые и незасчитанные чеки в сумму не входят, откат награды приходит отрицательной записью в ledger и уменьшает баланс сам собой. Слагаемые наружу не отдаются: второе всегда нулевое, потому что кэшбэка за покупку в продукте нет — синтетика, симуляция и демо шлют `points_earned = 0`, а поле в `receipts` существует только на случай, когда чек с баллами придёт из кассы через `POST /receipts`.

История — последние `limit` (1..200, по умолчанию 50) записей `reward_ledger` в порядке `created_at DESC, id DESC`. Каждая запись получает человеческий заголовок по `kind` и уточнение: для `ref_type='challenge'` — `copy_title` челленджа, для `ref_type='receipt'` — сумма чека, иначе пусто.

Справочник `rules` — таблица «за что начисляем» из §6 и §8: XP берётся из констант, диапазон баллов у челленджа — `REWARD_POINTS_MIN … REWARD_POINTS_MAX_WEEKLY`, у реферала — из `REFERRAL_REWARDS` по видам с ненулевой наградой пригласившему. Фронт эти числа не хранит, а показывает пришедшие.
