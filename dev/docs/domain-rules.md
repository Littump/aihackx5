...
```
points_from_rewards  = Σ reward_ledger.points_delta                       # игровые награды
points_from_receipts = Σ (points_earned − points_spent) по counted-чекам  # баллы кассы X5
points_balance       = points_from_rewards + points_from_receipts
```

Возвращённые и незасчитанные чеки в сумму не входят, откат награды приходит отрицательной записью в ledger и уменьшает баланс сам собой. В демо касса баллы не начисляет (`points_earned = 0` у синтетики и симуляции), поэтому баланс растёт только за выполненные цели и рефералов — это ожидаемое поведение, а не дефект.
