...
> Статус: **принято**. Карта того, что переработка LLM-планировщика из
> [ml-solution-architecture.md](ml-solution-architecture.md) меняет в уже зафиксированной архитектуре и
> продукте. Продуктовые/архитектурные документы уже синхронизированы (`docs/product-analysis/` PRD §6/§7,
> PRODUCT §5.2; `dev/docs/decisions.md` #7/#15; backlog эпик E14); **код** внедряется отдельно по задачам
> E14 (порядок — §8), поэтому здесь это по-прежнему карта изменений, а не сами правки кода.
...
Добавлено новое решение #15 «LLM-планировщик + детерминированный валидатор + fallback» (decision #7 уточнён).
...
| `app/features/challenges/economics.py` | добавить маппинг `reward_level → доля от max_reward_rub` (новая функция, формула прежняя) |
...
| `app/features/challenges/models.py` | новые модели `ChallengePlan` (`steps[]` из `ChallengeStep`), `PlannerInput` (с `previous_plans`), `PreviousPlan`, `ChurnRisk`, `RewardLevel`, `RewardKind` |
...
- `ChallengeDetail` (в `dto.py`) получает `sku_refs`, `reward_level`, `plan_source`.
...
- `reward_level → доля` — фиксируем таблицей (0/40/70/100 %) или калибруем симуляцией?
...
- ~~Каталог SKU: скрейпим реальные товары X5 или берём синтетический набор?~~ **Решено:** скрейпим реальные товары `perekrestok.ru` через `scripts/scrape_x5_catalog.py` (nodriver, обход WAF); 360 реальных SKU в `dev/backend/app/ml/data/perekrestok_catalog.json`.
...
|---|---|---|
| Один hero next action на главном экране (не набор равнозначных CTA) | демо §8; `domain-rules.md` §7 | Пользователю показан ровно `steps[0]`; `steps[1]` — внутренний сигнал (не в UI), следующий hero планируется заново (§5, §7, §10 arch) |
| LLM «получает готовые факты и числа, не назначает скидки и rewards» | демо §4 | LLM выбирает только `reward_kind` и ординальный `reward_level`; рубли/XP считает код (Economics/Ladder) |
